...
비키니시티는 바닷속에 위치한 활기찬 도시에요. 각 지역은 고유한 분위기와 문화·명소를 가지고 있으며, 주민들과 관광객들이 해저 도시 곳곳을 오가며 생활해요.

**비키티운송**에서 공식 버스 예약 서비스를 출시했어요! 스폰지밥과 뚱이가 편하게 버스를 예매할 수 있도록 새로운 예약 앱을 만들어주세요! 🧽✨

### 주요 지역

비키니시티는 11개의 정류장으로 구성되어 있어요:

- **비키니 시티** (Bikini Bottom) - 환승역
- **플로터스 묘지** (Floater's Cemetery)
- **버블타운** (Bubble Town)
- **뉴 켈프 시티** (New Kelp City)
- **글러브월드** (Glove World)
- **메롱시티** (Rock Bottom)
- **징징빌라** (Tentacle Acres)
- **비키니 환초** (Bikini Atoll)
- **켈프 숲** (Kelp Forest)
- **구-라군** (Goo Lagoon)
- **해파리 초원** (Jellyfish Fields)

자세한 노선 정보와 요금 체계는 [STATION_LINE_INFO.md](./STATION_LINE_INFO.md)를 참고하세요.
...
비키니시티의 버스 요금 체계는 **정거장 수·탑승 노선·환승 횟수**에 따라 계산됩니다.

### 1. 기본요금 (Base Fare)

비키티운송 버스에 탑승하면 가장 먼저 **기본요금**을 내요.

기본요금은 그 노선에서 **최대 2정거장까지 이동**할 수 있는 비용이에요.

- 예: 시티선 기본요금이 **10₴**이라면 → 탑승 후 **최대 2정거장**은 추가 비용 없이 이동 가능

### 2. 추가요금 (Extra Fare)

기본요금으로 커버할 수 있는 **2정거장을 초과해서 이동할 때** 남은 거리만큼 추가요금이 붙어요.

- 예: 기본요금 10₴, 추가요금 2₴, 3정거장 이동
  - 계산: 기본요금(10₴) + 추가요금 1회(2₴) = **12₴**

정거장 수가 많아질수록 추가요금이 누적될 수 있어요.

### 3. 환승 할인 (Transfer Discount)

비키니시티는 주민들을 위해 환승 시 할인 혜택을 제공해요.

**중요 규칙:**

- ✅ 환승 할인은 **"환승해서 새로 타는 노선"**의 정책이 적용돼요
- ✅ 환승할 때 타는 버스의 **할인율**을 받아요
- ✅ 환승할인은 **1회와 2회까지만** 적용 (3회 환승부터는 추가 할인 없음)
- ✅ **"환승 이전 노선"**에는 할인 적용되지 않아요

### 노선별 요금

| 노선   | 기본요금 | 추가요금 (정거장당) | 1회 환승 할인 | 2회 이상 환승 할인 |
| ------ | -------- | ------------------ | ------------- | ------------------ |
| 시티선 | 10₴      | 2₴                 | 10%           | 20%                |
| 외곽선 | 25₴      | 8₴                 | 15%           | 25%                |
| 투어선 | 15₴      | 5₴                 | 15%           | 20%                |
...
1. **시티선 탑승** → 3정거장 이동
   - 3정거장 = 기본요금 범위 초과(2정거장)
   - 요금: **10₴ + 2₴ = 12₴**

2. **투어선으로 환승** → 1정거장 이동 후 도착
   - 2정거장 → 기본요금(추가요금 없음)
   - 기본 15₴의 **15% 할인 적용**
   - 요금: 15₴ × 0.85 = **12.75₴**

3. **전체 합계:** 12₴ + 12.75₴ = **24.75₴**

### 쿠폰 종류

| 쿠폰코드     | 이름          | 할인 내용                    | 최대 소지 | 적용 조건              |
| ------------ | ------------- | ---------------------------- | --------- | ---------------------- |
| `PEARL_PASS` | 진주패스 🦪   | 모든 노선 기본요금 2₴ 할인   | 3개       | 없음 (전 노선 적용)    |
| `GARY_NIGHT` | 달팽이패스 🐌 | 야간(21시~05시) 전체 15% 할인 | 2개       | 21시~05시 탑승 시      |
| `TOUR_FUN`   | 투어패스 🎢   | 투어선 전용 30% 할인         | 5개       | 투어선 이용 시만       |
...
비키티운송 버스의 모든 좌석은 **행(숫자) + 열(영문)**으로 표기돼요.

- 숫자(1~6) → **버스 앞에서 뒤로 내려가는 순서**
- 알파벳(A, B, C, D) → **좌 → 우 방향 순서**

예: `1A`, `1B`, `1C`, `1D`, `2A`, `2B`, ..., `6A`, `6B`, `6C`, `6D`

### 좌석 상태

각 좌석은 실시간으로 아래 상태 중 하나를 가질 수 있어요.

| 상태              | 설명           |
| ----------------- | -------------- |
| **Available**     | 예약 가능 상태 |
| **Reserved**      | 이미 예약된 좌석 |
| **Selected**      | 예매 중 선택한 좌석 |
